<template>
    <IonModal :is-open="isOpen" @didDismiss="closeModal">
        <ion-header>
            <ion-toolbar>
                <ion-title for="" class="text-primary fw-bolder">ADD HAZARDS / RISKS</ion-title>
        ...
        <ion-content class="ion-padding" style="--padding-bottom: 0; height: 100%;">
            ...
            <ConfirmationAlert ref="confirmationAlert" @confirmed="onConfirmed" @cancelled="onCancelled"
                header="RISK AND OPPRTUNITY" message="Do you want to add risks?" confirmText="Yes"
                :data="selectedData" />
        </ion-content>
    ...
    methods: {
        ...
        async onConfirmed(data) {
            ...
            try {
                ...
                const response = await this.generalController.storeItem('risk-and-opportunity/add-risk', data);
                await this.$showMessageBox("RISK REGISTED ADDED", response.data.data);
